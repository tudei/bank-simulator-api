...
class Transfers_Controller(SQLite_Connector):
    ...
    def create_new(self, id_sender: int, id_receiver: int ,amount: float, 
        ...
        transfer_status = self.transfer_amount(id_sender, id_receiver, amount)
    ...
    def transfer_amount(self, id_sender: int,id_receiver: int, amount: float) -> bool:
        sender = user_controller.get_user(id_sender)[0]
        reciver = user_controller.get_user(id_receiver)[0]
